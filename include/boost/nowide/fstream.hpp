...
namespace detail {
    ...
    public:
        ...
        {}
        explicit fstream_impl(const char* file_name, std::ios_base::openmode mode = T_StreamType::mode) : fstream_impl()
        {
            open(file_name, mode);
        }
        ...
        explicit fstream_impl(const filesystem::path& file_name, std::ios_base::openmode mode = T_StreamType::mode) :
            fstream_impl()
        {
            open(file_name, mode);
        }
    ...
    protected:
        ...
        fstream_impl(fstream_impl&& other) noexcept : base_buf_holder(std::move(other)), stream_base(std::move(other))
        {
